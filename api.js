...
app.use((req, res, next) => {
  res.setHeader('X-Powered-By', 'Criminal-Minds-API-v2.0');
...
app.get('/api/health', async (req, res) => {
  try {
    ...
    res.json({
      ...
      responseTime: Date.now() - res.getHeader('X-Response-Time')
    });
  } catch (error) {
...
app.post('/api/auth/register', authLimiter, async (req, res) => {
  try {
    ...
    console.log('📝 Tentativa de registro:', { fullName, email, detectiveName });
...
const server = app.listen(PORT, async () => {
  try {
    ...
    console.log('⚡ Funcionalidades melhoradas v2.0:');
    ...
    console.log('   • ✅ Logging detalhado de requisições');
    console.log('   • ✅ Middleware de performance');
